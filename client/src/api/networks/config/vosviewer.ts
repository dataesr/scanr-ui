...
export function configGetItemPage(source: string, model: string, key: string): string {
  const targetUrl = conf[source][model]?.url_page
...
export function configGetItemSearch(query: string, source: string, model: string, key: string, integration: string): string {
  const itemField = conf[source][model].field + ".keyword"
  const targetUrl = conf[source][model].url_search
...
export function configCreate(source: string, model: string): NetworkConfig {
  const templates = {
    item_description: configGetItemDescription(source, model),
    link_description: configGetLinkDescription(model),
  }
  const terminology = conf?.[model]?.terminology
...
export function infoCreate(source: string, query: string, model: string): NetworkInfo {
